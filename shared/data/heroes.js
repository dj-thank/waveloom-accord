...
export const HEROES = Object.freeze([
  hero({
    id: 'zairu', name: 'ザイル', role: 'frontline', subtype: '高機動突入型', maxHp: 575, color: '#ef9d43',
    ...
    abilities: {
      ...
      ability1: action('ability1', 'toubyou', '投錨', 'anchor_launch', { cooldownSec: 11, castSec: 0.35, rangeM: 28, radiusM: 4, damage: 40, slowMult: 0.7, durationSec: 2.5 }),
  ...
  hero({
    id: 'baraga', name: 'バラガ', role: 'frontline', subtype: '近距離制圧型', maxHp: 700, color: '#c96d3d', moveSpeedMult: 0.94,
  ...
  hero({
    id: 'vesta', name: 'ヴェスタ', role: 'frontline', subtype: '中遠距離制御型', maxHp: 550, color: '#e7c95b',
  ...
  hero({
    id: 'nuedori', name: 'ヌエドリ', role: 'frontline', subtype: '妨害・変則型', maxHp: 525, color: '#755b9c',
  ...
  hero({
    id: 'sedora', name: 'セドラ', role: 'frontline', subtype: '陣地・護送型', maxHp: 650, color: '#8f7b4f', moveSpeedMult: 0.95,
  ...
  hero({
    id: 'shiomaneki', name: 'シオマネキ', role: 'frontline', subtype: '波状突撃型', maxHp: 600, color: '#397f9f', moveSpeedMult: 0.92,
    ...
    abilities: {
      secondary: action('secondary', 'naminori', '波乗り', 'dash', { cooldownSec: 7, rangeM: 8, shield: 35 }),
      ability1: action('ability1', 'uneri', 'うねり', 'team_wave', { cooldownSec: 13, rangeM: 16, radiusM: 5, durationSec: 4, moveSpeedMult: 1.25, shield: 40, pushM: 3 }),
      ability2: action('ability2', 'shiogaeshi', '潮返し', 'cone_blast', { cooldownSec: 9, rangeM: 8, damage: 45, pushM: 5 }),
      ultimate: action('ultimate', 'michi', '大潮「満ち」', 'team_wave', { rangeM: 24, radiusM: 8, durationSec: 6, moveSpeedMult: 1.35, shield: 75, pushM: 7, ultCost: 100 }),
    },
  }),

  hero({
    id: 'asagi', name: 'アサギ', role: 'damage', subtype: '汎用中距離型', maxHp: 250, color: '#4f9dbc',
    ...
    abilities: {
      ...
      ability2: action('ability2', 'tsugiashi', '継ぎ足', 'dash', { cooldownSec: 12, rangeM: 8, durationSec: 0.9 }),
      ultimate: action('ultimate', 'sarashibi', '一斉標定「晒し灯」', 'team_reveal', { castSec: 1.2, durationSec: 8, damageMult: 1.28, ultCost: 100 }),
    },
  }),
  hero({
    id: 'shirasagi', name: 'シラサギ', role: 'damage', subtype: '長距離精密型', maxHp: 200, color: '#c9e6eb',
  ...
  hero({
    id: 'tsubakuro', name: 'ツバクロ', role: 'damage', subtype: '高機動遊撃型', maxHp: 200, color: '#53b886', moveSpeedMult: 1.08,
  ...
  hero({
    id: 'hokuchi', name: 'ホクチ', role: 'damage', subtype: '近距離戦闘型', maxHp: 300, color: '#db5537', moveSpeedMult: 0.98,
  ...
  hero({
    id: 'botan', name: 'ボタン', role: 'damage', subtype: '範囲制圧型', maxHp: 250, color: '#d75e8e', moveSpeedMult: 0.96,
  ...
  hero({
    id: 'ankou', name: 'アンコウ', role: 'damage', subtype: '特殊戦術型', maxHp: 250, color: '#4d6a90',
    ...
    abilities: {
      ...
      ability1: action('ability1', 'tsuridama', '釣り灯', 'seeking_blast', { cooldownSec: 10, rangeM: 35, damage: 70, resourceCost: 25 }),
      ability2: action('ability2', 'mizuheri', '水辺滑行', 'dash', { cooldownSec: 9, rangeM: 8 }),
      ultimate: action('ultimate', 'shinkainogyoretsu', '深海の行列', 'homing_barrage', { durationSec: 6, rangeM: 45, damage: 45, count: 6, ultCost: 100 }),
    },
  }),

  hero({
    id: 'tsuzuri', name: 'ツヅリ', role: 'support', subtype: '高出力回復型', maxHp: 225, color: '#e3b34e',
    passive: { name: '繕い針', kind: 'stored_heal', resource: { id: 'needles', name: '針数', max: 12, initial: 12 } },
    weapon: weapon('tsuzuri_light_needle', '灯針', { type: 'healing_projectile', damage: 24, allyHealStored: 60, headshotMult: 1.75, rps: 1.67, magSize: 12, reloadSec: 2.5, maxRangeM: 40, falloffStartM: 30, falloffEndM: 40, projectileSpeedMps: 70 }),
    abilities: {
      secondary: action('secondary', 'itokuri', '糸繰り', 'ammo_restore', { ammoPerSec: 4, moveSpeedMult: 0.8 }),
      ability1: action('ability1', 'tsuzuriwatari', '綴り渡り', 'ally_grapple', { cooldownSec: 11, castSec: 0.25, rangeM: 28, storedHeal: 60 }),
  ...
  hero({
    id: 'karakasa', name: 'カラカサ', role: 'support', subtype: '護衛・防御型', maxHp: 225, color: '#d55454',
    ...
    abilities: {
      secondary: action('secondary', 'ukenagashi', '受け流しの傘', 'projectile_guard', { cooldownSec: 5, durationSec: 2.5, damageTakenMult: 0.35, resourceCost: 25 }),
      ability1: action('ability1', 'kasasuberi', '傘滑り', 'air_dash', { cooldownSec: 9, rangeM: 7, shield: 25 }),
      ability2: action('ability2', 'kasauch', '傘打ち', 'cone_blast', { cooldownSec: 8, rangeM: 5, damage: 45, pushM: 4 }),
      ultimate: action('ultimate', 'senbonkasa', '千本傘', 'team_guard', { castSec: 0.6, durationSec: 4, damageTakenMult: 0.45, ultCost: 100 }),
    },
  }),
  hero({
    id: 'shirabe', name: 'シラベ', role: 'support', subtype: '攻撃支援型', maxHp: 225, color: '#a66fc2',
    passive: { name: '調弦', kind: 'linked_ally_resource', resource: { id: 'harmony', name: '和音', max: 100, initial: 25 } },
    ...
    abilities: {
      ...
      ability1: action('ability1', 'waon', '和音開放', 'ally_damage_buff', { cooldownSec: 10, rangeM: 30, durationSec: 5, damageMult: 1.2, resourceCost: 40 }),
  ...
  hero({
    id: 'hibari', name: 'ヒバリ', role: 'support', subtype: '高機動支援型', maxHp: 200, color: '#e9a94c', moveSpeedMult: 1.08,
    ...
    abilities: {
      ...
      ability1: action('ability1', 'wataribi', '渡り火', 'healing_trail', { cooldownSec: 10, rangeM: 10, durationSec: 6, healPerSec: 35, damagePerSec: 12 }),
      ability2: action('ability2', 'hibariage', '雲雀上げ', 'leap_heal', { cooldownSec: 9, radiusM: 5, heal: 55 }),
      ultimate: action('ultimate', 'watarinooohi', '渡りの大火', 'healing_trail', { rangeM: 24, radiusM: 4, durationSec: 12, healPerSec: 55, damagePerSec: 20, ultCost: 100 }),
    },
  }),
  hero({
    id: 'kazura', name: 'カズラ', role: 'support', subtype: '回復転換型', maxHp: 250, color: '#679653', moveSpeedMult: 0.97,
